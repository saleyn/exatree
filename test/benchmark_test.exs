defmodule AtreeBenchmark do
  ...
  @age_ranges ["18-34", "35-49", "50+"]  # Age ranges - expanded for more diversity in matches
  ...
  def run do
    ...
    matched_count = Enum.count(results, fn r   -> r > 0 end)  # Calculate statistics
    ...
    IO.puts(
      "Total time:                        #{:io_lib.format("~.3f", [match_time / 1_000_000])} s"
    )
  ...
  defp generate_orders(count) do
    1..count
    |> Enum.map(fn i ->
      age_idx = rem(i, 3) # Vary age range to create both matches and mismatches
  ...
  defp generate_impressions(count) do
    1..count
    |> Enum.map(fn i ->
      age_cycle = rem(i, 10) # Create a mix of matching and non-matching impressions

      age_value =            # Age values: 8 out of 10 will fall within order ranges, 2 will not
        case age_cycle do
          # Matches "18-34"
          0 -> "25"
          # Matches "35-49"
          ...
          # Matches "50+"
          7 -> "52"
          # Below "18-34" - no age match
          8 -> "10"
          # Above "50+" - no age match
          9 -> "99"
        end
